<template>
	<div class="flex flex-col">
		<nuxt-link to="/">
			<h2 class="text-gray-600 font-bold m-4 px-2 text-2xl hover:text-gray-800 transition-colors duration-150">
				Vue Components Library
			</h2>
		</nuxt-link>
		<div class="m-4 py-4 px-4 flex justify-center gap-4 bg-gray-200 rounded flex-wrap">
			<nuxt-link class="hover:text-teal-500 text-sm text-gray-600 px-1" to="/about">
				About
			</nuxt-link>
			<a
				target="_blank"
				class="hover:text-teal-500 text-sm text-gray-600 px-1"
				href="https://github.com/jpokan/n-components"
				>Github</a
			>
			<a
				target="_blank"
				class="text-sm hover:text-teal-500 font-semibold text-gray-800 px-1"
				href="https://ko-fi.com/jpokan"
			>
				Donate
			</a>
		</div>
		<div class="overflow-y-scroll scrolling-bar">
			<details v-for="(type, index) in components" :key="index" open class="mx-4 my-1">
				<summary
				...
				<ul>
					<nuxt-link v-for="(component, subindex) in type" :key="subindex" :to="component.path">
						<li
...
<script>
	export default {
		...
		created() {
			...
			// Return pages routes of components directory
