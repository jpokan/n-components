<template>
  <button
    class="text-gray-900 font-medium px-8 py-4 border-gray-500 border-2 bg-gray-200 rounded-full outline-none focus:outline-none"
    @mouseenter="renderEnter($event)"
    @mousemove="renderMagnet($event)"
    @mouseleave="renderLeave($event)"
  >
    <span
      class="pointer-events-none flex items-center h-full w-full justify-center transition-transform transform duration-75"
      >{{ text }}</span
    >
  </button>
</template>
